...
export class FacadeRenderer {
  ...
  private drawDimensions(rx: number, ry: number, rw: number, rh: number, W: number, H: number) {
    ...
    // Зона тапа на «600 мм» — центр размерной линии, расширенный по обе стороны
    this.dimRects.push({ axis: 'width',  x: rx + rw / 2 - 50, y: wy - 14, w: 100, h: 28 });
    ...
    this.dimRects.push({ axis: 'height', x: hx - 14, y: ry + rh / 2 - 50, w: 28, h: 100 });
  ...
  private hitTest(x: number, y: number): Hit {
    ...
    // 2. Попадание в размерную надпись (ширина/высота)
    for (const d of this.dimRects) {
      if (x >= d.x && x <= d.x + d.w && y >= d.y && y <= d.y + d.h) {
        return { kind: 'dim', axis: d.axis };
      }
    }
